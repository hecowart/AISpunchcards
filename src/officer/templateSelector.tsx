...
export function TemplateSelector({ onTemplateSelect, selectedTemplate }: TemplateSelectorProps) {
  ...
  return (
    <div className="template-selector">
      <h3>{t("choose_a_template_optional")}</h3>
      <div className="template-options">
        ...
        >
          <div className="template-header">
            <h4>{t("start_from_scratch")}</h4>
          </div>
          <p>{t("create_a_new_event_with_blank_fields")}</p>
        </div>
            ...
            >
              ...
              <div className="template-preview">
                ...
                {template.description && (
                  <p><strong>{t("description")}</strong> {template.description.length > 100 
      ...
      {templates.length === 0 && (
        <p className="no-templates">{t("no_templates_available_create_your_first_template_")}</p>
      )}
    </div>
  );
}